...
<template>
  <div class="cv-bg min-h-screen bg-neutral-100 text-neutral-900 overflow-x-hidden print:bg-white">
    <div class="page-wrap mx-auto w-full max-w-[980px] px-4 py-8 md:px-8 md:py-12">
      <article class="sheet bg-white shadow-[0_24px_70px_rgba(0,0,0,0.10)] ring-1 ring-black/5 rounded-2xl px-6 py-8 md:px-10 md:py-12">
        <!-- Header -->
        <header class="space-y-4">
          ...
          <div class="text-sm text-neutral-800 space-y-1 break-words">
            <div>
              邮箱：
        ...
        <hr class="my-7 border-neutral-200" />

        <!-- Overview -->
        <section class="space-y-2">
          <h2 class="text-lg font-black tracking-tight">概览</h2>
          <div class="text-[15px] leading-relaxed text-neutral-800 space-y-1">
        ...
        <!-- Skills -->
        <section v-if="resumeGroups.length" class="space-y-4">
          <h2 class="text-lg font-black tracking-tight">技能</h2>

          <div class="space-y-5">
            <div v-for="group in resumeGroups" :key="group.tag" class="space-y-2">
              <h3 class="text-sm font-black text-neutral-800">
                {{ group.tag }}
              </h3>
              <ul class="list-disc pl-5 text-[15px] leading-relaxed text-neutral-800 space-y-1">
            ...
            <div v-for="project in enabledProjects" :key="project.id" class="space-y-1">
              <div class="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1">
                <a
                  :href="project.url"
                  target="_blank"
                  class="text-[15px] font-black tracking-tight underline underline-offset-4 decoration-neutral-300 hover:decoration-neutral-600"
                >
                  {{ project.title }}
                </a>
                <div class="text-xs font-semibold text-neutral-500 shrink-0">
                  {{ project.date }}
                </div>
              </div>

              <p class="text-[15px] text-neutral-800 leading-relaxed">
                {{ project.description }}
              </p>

              <p v-if="project.stack" class="text-xs text-neutral-500">
                {{ project.stack }}
              </p>
            </div>
          </div>
        ...
        <section class="space-y-2">
          ...
          <div class="text-[15px] leading-relaxed text-neutral-800">
            <div class="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1">
              <span class="font-semibold">双非本科 · 数字媒体</span>
              <span class="text-xs text-neutral-500">2021 — 2026</span>
            </div>
          </div>
        </section>
        ...
        <footer class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between text-xs text-neutral-500">
    ...
    <Teleport to="body">
      ...
      <button
        type="button"
        class="no-print fixed bottom-[calc(1.5rem+env(safe-area-inset-bottom))] right-[calc(1.5rem+env(safe-area-inset-right))] z-[9999] h-12 w-12 rounded-full bg-neutral-900 text-white shadow-lg shadow-black/20 hover:shadow-xl hover:shadow-black/25 transition"
        aria-label="打开简历显示控制"
        @click="toggleDrawer"
      >
        <Icon name="ph:sliders-horizontal-bold" class="text-xl mx-auto" />
      </button>
      ...
      >
        <div v-if="drawerOpen" class="no-print fixed inset-0 z-[10000]">
          <button
            type="button"
          ...
          <aside
            class="absolute right-0 top-0 h-full w-[min(460px,100vw)] bg-white shadow-2xl ring-1 ring-black/10"
            role="dialog"
            aria-modal="true"
          >
            <div class="flex h-full flex-col">
              <div
                class="flex items-start justify-between gap-4 p-5 border-b border-neutral-200/70"
              >
                <div class="space-y-1">
                ...
                <div class="flex items-center gap-2">
                  <button
                    type="button"
                    class="h-9 rounded-lg px-3 text-xs font-bold ring-1 ring-neutral-200 hover:ring-neutral-300 transition"
                    @click="resetControls"
                  >
                    全选
                  </button>
                  <button
                    type="button"
                    class="h-9 w-9 rounded-lg ring-1 ring-neutral-200 hover:ring-neutral-300 transition"
                    aria-label="关闭"
                    @click="closeDrawer"
                  >
                    <Icon name="ph:x" class="text-lg mx-auto opacity-70" />
                  </button>
                </div>
              </div>
              ...
              <div class="flex-1 overflow-auto p-5 space-y-8">
                ...
                <section class="space-y-3">
                  <div class="flex items-end justify-between gap-3">
                    <h3 class="text-[11px] font-black tracking-[0.4em] uppercase text-neutral-500">
                      Resume
                    </h3>
                    <button
                      type="button"
                      class="text-xs font-semibold text-neutral-500 hover:text-neutral-800 transition"
                      @click="printResume"
                    >
                      打印 / 导出
                    </button>
                  ...
                  <div class="rounded-xl ring-1 ring-neutral-200/70 overflow-hidden">
                    <div
                      class="grid grid-cols-[auto,1fr,auto] items-center gap-x-3 px-4 py-2 bg-neutral-50 text-[10px] font-black uppercase tracking-[0.35em] text-neutral-500"
                    >
                      <span>On</span>
                      <span>Label</span>
                    ...
                    <div class="divide-y divide-neutral-200/70">
                      ...
                      >
                        <label class="relative inline-flex cursor-pointer items-center">
                          ...
                          />
                          <span
                            class="h-5 w-9 rounded-full bg-neutral-200 peer-checked:bg-neutral-900 transition"
                          />
                          <span
                            class="absolute left-0.5 top-0.5 h-4 w-4 rounded-full bg-white shadow-sm transition peer-checked:translate-x-4"
                          />
                        </label>
                        <span class="text-sm text-neutral-800 leading-snug">
                          {{ item.label }}
                        </span>
                        <span
                ...
                <section class="space-y-3">
                  ...
                  <div class="rounded-xl ring-1 ring-neutral-200/70 overflow-hidden">
                    <div
                      class="grid grid-cols-[auto,1fr] items-center gap-x-3 px-4 py-2 bg-neutral-50 text-[10px] font-black uppercase tracking-[0.35em] text-neutral-500"
                    >
                      <span>On</span>
                      <span>Title</span>
                    </div>
              ...
              <div class="p-5 border-t border-neutral-200/70">
                <p class="text-xs text-neutral-500">
