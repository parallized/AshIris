...
const projectPool = ref<ProjectItem[]>([
  ...
  // },
  {
    ...
    enabled: true,
    title: "万级游戏社区服务器全生命周期研发与运营",
    summary: "MC 周边游戏开发，累计 10W+ 注册用户，规模化盈利",
    url: "https://github.com/parallized",
    date: "2020 - 至今",
    stack: "Java / 游戏设计 / 分布式架构 / UI UX 全套设计",
    roles: ["全栈开发", "产品设计", "后端开发"],
    tags: ["高并发治理", "商业闭环", "危机管控", "产品决策"],
    evidence: [
      "累计注册用户 10W+，峰值 DAU 2000+，月纯利 20k+，持续稳定运营 2 年",
      "通过 Redis 多级缓存与分布式拆分，在 DAU 2000+ 峰值下保持系统延迟 < 100ms。",
    ],
    star: {
      situation: "从零构建支撑万人规模的玩家社区服务器，并在技术架构、产品设计与商业运营之间实现闭环。",
      task: "作为唯一负责人，统筹从技术架构、UI/UX 设计到数值策划与商业运营的全流程，实现可持续盈利",
      actions: [
        "采用 Java + Redis 分布式架构，设计三级缓存与服务器分流策略，确保高并发场景下的系统稳定性。",
        "独立完成全套 UI/UX 设计，基于用户行为数据分析优化关键转化路径（注册流从 5 步优化至 2 步）。",
        "建立成熟的危机处理机制（补偿方案设计、业务 24 小时快速恢复），将突发故障的用户流失率控制在 3% 以内。",
      ],
      results: [
        "系统稳定支撑万人社区，实现月纯利 20k+ 的商业目标，运营至今已超 5 年。",
        "产品决策驱动的留存率优化，活跃用户月留存 > 60%，验证了「一人全栈 + 数据驱动」的产品方法论。",
      ],
    },
  },
  {
    ...
    summary: "基于 MCP/RAG/SDD 的意图驱动开发范式，单人日均交付量达传统 5 人团队水平。",
    url: "https://github.com/parallized",
    date: "2025 - 至今",
    stack: "Claude Code / Codex / MCP / RAG / Vue 3 / Nuxt 4",
    roles: ["AI-Native 开发", "Agent 开发", "前端开发"],
    tags: ["交付效能 5-10x", "意图驱动", "上下文治理", "代码审计"],
    evidence: [
      "2024-2025 年间独立交付 10+ 个覆盖知识图谱、游戏辅助、空间可视化的全栈应用，平均原型周期 < 1 天。",
      "建立基于 MCP 的 SDD 流转体系，代码评审与任务回写全自动化。",
    ],
    star: {
      situation: "传统开发范式在处理长周期、多上下文项目时效率受限，AI 辅助开发面临幻觉与上下文碎片化挑战。",
      task: "建立一套可验证的 AI 意图驱动极速交付方法论，使单人开发达到团队级产出。",
      actions: [
        "深度集成 Claude Code/Codex 与自研 MCP 工具链，实现任务创建→派发→执行→回写的自动化闭环。",
        "开发多层级 RAG 检索系统，将项目长短时记忆分层注入 AI 上下文，解决幻觉与注意力衰减。",
        "建立严格的人工代码审计流程，确保 AI 生成逻辑符合生产级安全标准（Typecheck/E2E 双重校验）。",
      ],
      results: [
        "开发效能较传统模式提升 5-10 倍，实现 2 小时内从 0 到 1 交付样式深度优化的全栈系统。",
        "验证了 Vibe Coding 在复杂业务场景下的可落地性，形成可复制的「AI 驱动 + 人工审计」最佳实践。",
      ],
    },
  },
  {
    ...
    summary: "为 AI 编码场景独特设计的 Schema-Driven Development 看板系统，实现多 Worker 任务派发、状态管理与结构化报告回写。",
    ...
    evidence: [
      "已在 5+ 个项目中实际应用，支撑 Claude Code 与 Codex 双端并行任务执行。",
      "开发过程 100% 可追溯：每个任务的创建、执行、报告均有结构化记录。",
    ],
    star: {
      situation: "AI 编码工具缺少结构化的任务管理层，执行过程不可追溯，多 Agent 并行时上下文混乱。",
      task: "从 0 到 1 设计一套面向 AI 编码场景的 SDD 看板系统，打通「需求→任务→执行→回写」闭环。",
      actions: [
        "设计六态任务状态流（草稿/待办/队列中/进行中/已完成/已阻塞），统一多 Worker 执行视图与冲突解决。",
        "实现完整 MCP 工具集（任务查询、报告提交、标签管理、Worker 完成通知），支持跨 IDE 调用。",
        "构建可视化看板 UI，提供直观的任务构思、编排与优先级管理体验。",
      ],
      results: [
        "显著降低多任务并行时的认知负担与上下文切换成本，实现开发过程的 100% 可追溯性。",
        "验证了 SDD 工作流在真实项目中的实用价值，已在 5+ 个项目中持续使用。",
      ],
    },
  },
  {
    ...
    summary: "将 Notion 升级为 AI Agent 可直接执行任务的项目中枢，统一 Claude 与 Codex 双端指令体验。",
    ...
    evidence: [
      "开源项目，README 提供完整的初始化、执行、回写与截图流程。",
      "抽象为 Skill 形态，一条指令即可触发 AI 执行完整的任务工作流。",
    ],
    star: {
      situation: "团队在 Notion 管理任务，但 AI 执行结果无法自动回写，「任务写了但执行不闭环」的问题反复出现。",
      task: "设计可跨工具运行的自动化任务工作流，让 AI Agent 直接在 Notion 中领票、执行、上报。",
      actions: [
        "定义完整的状态流（待办→队列中→进行中→已完成/已阻塞），约束报告输出格式，确保可追溯性。",
        "组合 Notion MCP 与 REST API 双查询链路，解决 MCP 单点故障问题，提升任务发现稳定性。",
        "抽象为 Skill 形态，统一 Claude Code 与 Codex 两端的指令体验，降低使用门槛。",
      ],
      results: [
        "形成可复用的 AI 项目执行模板，从需求到交付的协作成本降低 70%。",
        "执行结果、标签和评论 100% 可回溯，为 Maple 的设计提供了核心经验积累。",
      ],
    },
  },
  {
    ...
    summary: "基于 Rust + Detour/NavMesh 的导航能力服务，提供标准化 RESTful API，支撑上层工具的路径与空间查询。",
    ...
    evidence: [
      "10+ 标准化 RESTful 接口（路径规划/射线检测/随机点/网格邻域），覆盖主流空间查询场景。",
      "完整 E2E 测试套件（e2e/run.js），确保接口级别的回归可靠性。",
    ],
    star: {
      situation: "逆向工程流水线需要一个高性能的导航能力服务，现有方案无法满足毫秒级响应与灵活扩展的需求。",
      task: "将底层 NavMesh 能力封装为可独立部署、标准化调用、自动化验证的 HTTP 服务。",
      actions: [
        "使用 Axum 搭建高性能 API 层，统一 POST JSON 请求约定，实现请求/响应的结构化处理。",
        "通过 Rust FFI 对接 C++ bridge 与 Detour 内核，在保留原生性能的同时获得 Rust 的安全保障。",
        "实现 health/config/path/random-point/cast-ray 等 10+ 核心接口，并编排自动化 E2E 测试套件。",
      ],
      results: [
        "完成 10+ 可用导航接口，路径规划响应时间 < 5ms，支撑上层自动化流程的实时空间查询需求。",
        "形成可独立运行的微服务层，便于后续接入更多客户端，已稳定服务于逆向工程流水线。",
      ],
    },
  },
  {
    ...
    evidence: [
      "线上持续运营：rune.parallized.cn，用户可直接体验完整功能。",
      "产品决策：按学习路径重构信息架构，将首次进入成本降低 50%+。",
    ],
    star: {
      situation: "学习资料分散在不同平台，上下文割裂导致学习效率低下，用户难以形成系统性知识体系。",
      task: "打造一个可持续演进的知识入口，兼顾结构化组织、浏览效率与内容可检索性。",
      actions: [
        "按学习路径重构信息架构，设计多维索引结构，将首次进入成本降低 50%。",
        "结合 AI 辅助内容生成与前端交互优化，提升检索命中率与回溯效率。",
        "以小步快跑方式持续验证页面结构和表达方式，累计迭代 30+ 个版本。",
      ],
      results: [
        "形成稳定的线上知识学习入口，可直接用于知识沉淀、学习路径规划与日常复盘。",
        "为后续 JD 定制化内容（强调技术/产品侧）提供可复用底座，验证了「知识产品化」的可行性。",
      ],
    },
  },
  {
    ...
    summary: "面向公会协作的 AI + MRT 战术信息系统，将排轴方案从个人经验沉淀为可共享可迭代的团队资产。",
    ...
    evidence: [
      ...
      "产品决策：将战术信息模块化，支持多人协作查看与快速迭代。",
    ],
    star: {
      situation: "团队排轴和战术同步完全依赖人工，信息更新慢且版本混乱，关键时刻信息可读性差。",
      task: "构建一个能快速协作、统一认知的战术信息系统，让团队在实战前 5 分钟内完成战术同步。",
      actions: [
        "将战术信息拆解为可复用模块化组件，支持多人并行查看、编辑与版本迭代。",
        "引入 AI 辅助生成与规则化处理引擎，将手工配置耗时减少 80%。",
        "围绕实战反馈持续调整信息层级与视觉权重，确保核心信息在高压场景下可一眼定位。",
      ],
      results: [
        "形成可复用的战术板工作流，团队战术同步时间从 30 分钟压缩至 5 分钟。",
        "将排轴方案从「个人经验」沉淀为「可共享资产」，降低团队协作摩擦。",
      ],
    },
  },
  {
    id: "ash-iris",
    enabled: true,
    title: "Ash Iris — 交互式简历与个人品牌站",
    summary: "支持暗黑模式、双栏布局、岗位模板筛选与 A4 打印的交互式简历系统，实现「一份底稿裂变多简历」。",
  ...
  },
  {
    ...
    star: {
      situation: "Unity 项目中内容整理与工程理解成本极高，开发者频繁在 IDE/浏览器/文档间切换上下文。",
      ...
      actions: [
        ...
        "以 Demo 场景验证完整的工具链与开发体验，确认方案可落地性。",
      ],
      results: [
        "验证了编辑器内 AI 助手的可行性，为后续产品化演进提供明确的技术路线图。",
  ...
  },
  {
    ...
    evidence: [
      "覆盖 Valorant / OW / War Thunder 等多款热门游戏的战术点位数据。",
      "统一的信息架构设计，支持跨游戏的 Lineup 内容组织与检索。",
    ],
    star: {
      ...
      actions: [
        ...
        "优化浏览路径与交互设计，确保实战前 30 秒内可获取关键 Lineup 信息。",
        "建立可扩展的数据结构，支持社区持续贡献新游戏与新地图的 Lineup 内容。",
      ],
      results: [
        "形成可持续更新的多游戏 Lineup 资源库，统一了碎片化的战术信息入口。",
        "沉淀了空间信息展示的可复用交互模式与数据组织规范。",
      ],
    },
  },
  {
    ...
    summary: "C++ 异步 TCP 通信内核 + Electron 桌面端的跨语言架构实践，验证底层服务与前端协同的最佳路径。",
    ...
    evidence: [
      "跨语言架构实践：C++ 异步通信内核处理并发连接 + Electron 桌面端提供可视化操作界面。",
      "自定义通信协议设计，保证数据传输的稳定性与低延迟。",
    ],
    star: {
      ...
      results: [
        "完成可用的 C++/Electron 混合架构方案，验证了底层服务与前端协同的最佳路径。",
        "积累 C++ 异步网络编程与 Electron 跨进程通信实践经验，为后续 Rust 服务化提供架构参考。",
      ],
    },
  },
]);
